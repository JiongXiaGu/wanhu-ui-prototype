...
function ChoiceInputDialogView({request,onDismiss,interactive,motionPhase}:{request:ChoiceInputDialogRequest;onDismiss:()=>void;interactive:boolean;motionPhase:MotionPhase}){
  ...
  useEffect(()=>{if(interactive){inputRef.current?.focus();inputRef.current?.select()}},[request.id,interactive]);
  useEffect(()=>{if(!interactive)return;const handleKey=(event:KeyboardEvent)=>{const target=event.target instanceof Element?event.target:null;const inChoiceSelect=Boolean(target?.closest('.ui-dialog-choice-select'));if((choiceOpen&&(event.key==='Escape'||event.key==='Enter'))||(inChoiceSelect&&event.key==='Enter'))return;if(event.key==='Escape'){event.preventDefault();event.stopPropagation();request.onCancel?.();onDismiss()}else if(event.key==='Enter'&&canConfirm){event.preventDefault();event.stopPropagation();request.onConfirm(trimmed,choice);onDismiss()}};window.addEventListener('keydown',handleKey,true);return()=>window.removeEventListener('keydown',handleKey,true)},[request,onDismiss,trimmed,choice,canConfirm,interactive,choiceOpen]);
