...
import { Mountain } from '../../ui/icons/runtime-icons.generated';
...
export function TerrainEditTool({ state, motionPhase = 'steady', dispatch, onExit }: TerrainEditToolProps) {
  ...
  const modeGroups: ToolModeGroup[] = [{
    id: 'terrain-edit-mode', label: '地形编辑模式', presentation: 'icon-label',
    items: [
      { id: 'raise', label: '抬高', icon: 'terrain-raise', active: state.terrainEditMode === 'raise', onClick: () => dispatch({ type: 'SET_TERRAIN_EDIT_MODE', mode: 'raise' }) },
      { id: 'lower', label: '降低', icon: 'terrain-lower', active: state.terrainEditMode === 'lower', onClick: () => dispatch({ type: 'SET_TERRAIN_EDIT_MODE', mode: 'lower' }) },
      { id: 'flatten', label: '整平', icon: 'terrain-flatten', active: state.terrainEditMode === 'flatten', onClick: () => dispatch({ type: 'SET_TERRAIN_EDIT_MODE', mode: 'flatten' }) },
      { id: 'smooth', label: '平滑', icon: 'terrain-smooth', active: state.terrainEditMode === 'smooth', onClick: () => dispatch({ type: 'SET_TERRAIN_EDIT_MODE', mode: 'smooth' }) },
      { id: 'slope', label: '坡面', icon: 'terrain-slope', active: state.terrainEditMode === 'slope', onClick: () => dispatch({ type: 'SET_TERRAIN_EDIT_MODE', mode: 'slope' }) },
    ],
  }];
  return (
    <>
      <LeftContextPanel as="section" ariaLabel="地形编辑参数" icon={Mountain} title="地形编辑" subtitle="塑形与标高" closeLabel="退出地形编辑" className={`terrain-edit-prototype motion-left-surface is-${motionPhase}`} bodyClassName="terrain-edit-prototype__body" onClose={onExit} dataAttributes={{ 'data-terrain-edit-mode': state.terrainEditMode }}>
        ...
        {state.terrainEditMode === 'slope' && (
          <LeftContextSection title="坡面参数" className="terrain-edit-mode-section">
            <div className="terrain-edit-metrics terrain-edit-metrics--slope">
              <span><small>起点高度</small><b>12.40 m</b></span><span><small>终点高度</small><b>18.60 m</b></span><span><small>预估坡度</small><b>7.2°</b></span>
            </div>
          </LeftContextSection>
        )}
      </LeftContextPanel>
      <div className={`tool-bottom-cluster terrain-edit-toolbar-cluster motion-bottom-surface is-${motionPhase}`} aria-label="地形编辑主控栏" aria-busy={motionPhase !== 'steady'}>
        <ToolActionBar ariaLabel="地形编辑操作栏" modeGroups={modeGroups} completeLabel="完成地形编辑" completeShortLabel="完成" completeKind="exit" commitGroupLabel="地形编辑任务" onComplete={onExit} />
      </div>
      <div className={`terrain-brush-preview terrain-brush-preview--${state.terrainEditMode}`} aria-hidden="true">
        ...
        <span className="terrain-brush-preview__cross terrain-brush-preview__cross--h" /><span className="terrain-brush-preview__cross terrain-brush-preview__cross--v" />
      </div>
    </>
  );
}
