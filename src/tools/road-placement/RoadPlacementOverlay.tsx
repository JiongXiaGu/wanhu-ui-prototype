...
function RoadParameterControl({
  ...
  return (
    <div className="road-placement-parameter-row">
      <span>{label}</span>
...
const MODE_COPY: Record<RoadDrawMode, { title: string; detail: string }> = {
  'smart-curve': {
    title: '智能曲线',
    detail: '自动平滑转折并辅助连接既有道路，适合连续街巷与自然转向。',
  },
  curve: {
    title: '曲线',
    detail: '手动控制道路曲线形态，适合河岸、园路与精确弯道。',
  },
  straight: {
    title: '直线',
    detail: '保持道路轴线笔直，适合城市中轴、坊巷与规则路网。',
  },
};
    ...
    <section className="tool-overlay road-placement-prototype" data-road-mode={drawMode}>
      ...
      <div className="tool-body">
        ...
        <section className="road-placement-parameters">
          ...
          <RoadParameterControl label="道路宽度" initial={6} step={1} suffix=" m" onDirty={onDirty} />
          <RoadParameterControl label="相对标高" initial={0} step={0.1} suffix=" m" onDirty={onDirty} />
          {drawMode !== 'straight' && (
            <RoadParameterControl label="曲线平滑" initial={60} step={5} suffix="%" onDirty={onDirty} />
          )}
        </section>
      </div>
    </section>
