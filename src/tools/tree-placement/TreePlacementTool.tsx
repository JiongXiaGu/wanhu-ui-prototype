...
interface TreePlacementToolProps {
...
export function TreePlacementTool({ state, motionPhase = 'steady', dispatch, onExit }: TreePlacementToolProps) {
  ...
  function selectVariant(variant: number) {
    setSingleSelected(true);
    dispatch({ type: 'SET_TREE_VARIANT', variant });
  ...
  return (
    <>
      <LeftContextPanel
        ...
        ariaLabel="树木放置参数"
        icon={Trees}
        title={state.treeSpeciesName}
        subtitle="树木放置"
        closeLabel="退出树木放置"
        className={'tree-placement-prototype motion-left-surface is-' + motionPhase}
        bodyClassName="tree-placement-prototype__body"
        onClose={onExit}
        dataAttributes={{ 'data-tree-placement-mode': state.treePlacementMode, 'data-tree-species': state.treeSpeciesId }}
      >
          ...
          <div className="tree-variant-rows" aria-label={state.treeSpeciesName + '树形样式'}>
            {TREE_VARIANT_ROWS.map((row, rowIndex) => (
              <div className="tree-variant-row" key={rowIndex}>
                {row.map((variant) => (
                  <button type="button" className={'tree-variant-button tree-variant-button--' + variant + ' ' + (state.treeVariant === variant ? 'is-active' : '')} aria-label={'树木样式 ' + String(variant).padStart(2, '0')} aria-pressed={state.treeVariant === variant} key={variant} onClick={() => selectVariant(variant)}>
                    <Trees aria-hidden="true" /><span>{String(variant).padStart(2, '0')}</span>
                  </button>
                ))}
              </div>
            ))}
      ...
      <div className={'tool-bottom-cluster tree-placement-toolbar-cluster motion-bottom-surface is-' + motionPhase} aria-label="树木放置主控栏" aria-busy={motionPhase !== 'steady'}>
        <ToolActionBar ariaLabel="树木放置操作栏" modeGroups={modeGroups} quickActions={singleActions} completeLabel="完成树木放置" commitGroupLabel="树木放置任务" onComplete={onExit} />
      </div>
      ...
      {state.treePlacementMode === 'brush' ? (
        <div className={'tree-brush-preview tree-brush-preview--variant-' + state.treeVariant} aria-hidden="true">
          ...
          <span className="tree-brush-preview__tree tree-brush-preview__tree--1"><Trees /></span>
          <span className="tree-brush-preview__tree tree-brush-preview__tree--2"><Trees /></span>
          <span className="tree-brush-preview__tree tree-brush-preview__tree--3"><Trees /></span>
          <span className="tree-brush-preview__tree tree-brush-preview__tree--4"><Trees /></span>
          <span className="tree-brush-preview__tree tree-brush-preview__tree--5"><Trees /></span>
        </div>
      ) : (
        <div className={'tree-single-preview ' + (singleMoved ? 'is-moved ' : '') + (singleSelected ? 'is-selected' : 'is-empty')} aria-hidden="true">
          ...
          {singleSelected && <>
            <Trees className={'tree-single-preview__tree tree-single-preview__tree--variant-' + previewVariant} style={{ rotate: singleRotation + 'deg', scale: singleScale / 100 }} />
            <span className="tree-single-preview__axis tree-single-preview__axis--x" />
            <span className="tree-single-preview__axis tree-single-preview__axis--z" />
          </>}
        </div>
      )}
    </>
