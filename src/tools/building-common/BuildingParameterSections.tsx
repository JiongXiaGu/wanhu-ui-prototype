...
function NumericParameter({ label, initial, min, max, step, suffix = '', onDirty }: {
...
export function BuildingPositionParameters({ onDirty }: { onDirty: () => void }) {
  return (
    <LeftContextSection title="位置调整" className="bp-mode-content">
      ...
      <NumericParameter label="旋转角度" initial={0} min={0} max={345} step={15} suffix="°" onDirty={onDirty} />
      <NumericParameter label="吸附距离" initial={4} min={0} max={10} step={0.5} suffix=" m" onDirty={onDirty} />
    </LeftContextSection>
  );
}
...
export function BuildingMassingParameters({ onDirty }: { onDirty: () => void }) {
  return (
    <LeftContextSection title="楼身调整" className="bp-mode-content">
      <NumericParameter label="楼层数量" initial={3} min={1} max={12} step={1} onDirty={onDirty} />
      <NumericParameter label="单层高度" initial={4.2} min={2.4} max={8} step={0.1} suffix=" m" onDirty={onDirty} />
      <SegmentRow label="柱网布局" items={['疏朗', '均衡', '紧凑']} defaultValue="均衡" onDirty={onDirty} />
      <NumericParameter label="楼层收分" initial={0.12} min={0} max={0.4} step={0.01} onDirty={onDirty} />
    </LeftContextSection>
  );
}
...
export function BuildingRoofParameters({ onDirty }: { onDirty: () => void }) {
  return (
    <LeftContextSection title="屋顶调整" className="bp-mode-content">
      ...
      <NumericParameter label="出檐尺度" initial={1.4} min={0} max={4} step={0.1} suffix=" m" onDirty={onDirty} />
      <NumericParameter label="翼角起冲" initial={0.45} min={0} max={1} step={0.05} onDirty={onDirty} />
    </LeftContextSection>
  );
}
