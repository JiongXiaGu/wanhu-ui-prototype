...
try {
  ...
  const materialFavoriteFilter = page.locator('.material-preset-workspace__rail .workspace-primary-rail__favorite');
  assert.equal(await materialFavoriteFilter.count(), 1, '材质方案收藏筛选必须保持原有按钮外观');
  assert.equal(await page.locator('.material-preset-workspace__rail .workspace-primary-rail__favorite-toggle').count(), 0, '材质方案收藏筛选不得渲染为开关轨道外观');
  assert.equal(await materialFavoriteFilter.getAttribute('aria-pressed'), 'false', '材质方案收藏筛选初始应关闭');
  await materialFavoriteFilter.click();
  await settle();
  assert.equal(await page.locator('.material-preset-workspace').getAttribute('data-material-favorite'), 'true', '材质方案收藏筛选必须可开启');
  await page.locator('.material-preset-workspace__rail-list').getByRole('button', { name: '木材', exact: true }).click();
  await settle();
  assert.equal(await page.locator('.material-preset-workspace').getAttribute('data-material-favorite'), 'true', '切换材质分类不得隐式关闭收藏筛选');
  assert.equal(await page.locator('.material-preset-workspace').getAttribute('data-material-preset-category'), 'wood', '收藏筛选应与材质分类正交组合');
  await page.screenshot({ path: `${out}/color-tool-material-favorite-button.png` }); report.screenshots.push('color-tool-material-favorite-button');
  await materialFavoriteFilter.click();
  await settle();
  assert.equal(await page.locator('.material-preset-workspace').getAttribute('data-material-favorite'), 'false', '材质方案收藏筛选必须可独立关闭');
  ...
  const buildingFavoriteFilter = buildingSchemeWorkspace.locator('.building-scheme-workspace__rail .workspace-primary-rail__favorite');
  assert.equal(await buildingFavoriteFilter.count(), 1, '建筑配色收藏筛选必须保持原有按钮外观');
  assert.equal(await buildingSchemeWorkspace.locator('.building-scheme-workspace__rail .workspace-primary-rail__favorite-toggle').count(), 0, '建筑配色收藏筛选不得渲染为开关轨道外观');
  assert.equal(await buildingFavoriteFilter.getAttribute('aria-pressed'), 'false', '建筑配色收藏筛选初始应关闭');
  await buildingFavoriteFilter.click();
  await settle();
  assert.equal(await buildingSchemeWorkspace.getAttribute('data-building-scheme-favorite'), 'true', '建筑配色收藏筛选必须可开启');
  await buildingSchemeWorkspace.locator('.building-scheme-workspace__rail-list').getByRole('button', { name: '素雅', exact: true }).click();
  await settle();
  assert.equal(await buildingSchemeWorkspace.getAttribute('data-building-scheme-favorite'), 'true', '切换配色风格不得隐式关闭收藏筛选');
  assert.equal(await buildingSchemeWorkspace.getAttribute('data-building-scheme-style'), 'elegant', '收藏筛选应与配色风格正交组合');
  await page.screenshot({ path: `${out}/color-tool-building-scheme-favorite-button.png` }); report.screenshots.push('color-tool-building-scheme-favorite-button');
  await buildingFavoriteFilter.click();
  await settle();
  assert.equal(await buildingSchemeWorkspace.getAttribute('data-building-scheme-favorite'), 'false', '建筑配色收藏筛选必须可独立关闭');
  ...
  const blueprintFavoriteFilter = blueprintWorkspace.locator('.blueprint-workspace__rail .workspace-primary-rail__favorite');
  assert.equal(await blueprintFavoriteFilter.count(), 1, '蓝图左 Rail 收藏筛选必须保持原有按钮外观');
  assert.equal(await blueprintFavoriteFilter.getAttribute('aria-pressed'), 'false', '蓝图收藏筛选初始应关闭');
  assert.equal(await blueprintWorkspace.locator('.blueprint-workspace__rail .workspace-primary-rail__favorite-toggle').count(), 0, '蓝图收藏筛选不得渲染为开关轨道外观');
  ...
  await settle();
  assert.equal(await blueprintWorkspace.getAttribute('data-blueprint-favorite'), 'true', '蓝图收藏筛选 必须进入 Favorite 状态');
  assert.equal(await blueprintFavoriteFilter.getAttribute('aria-pressed'), 'true', '蓝图收藏筛选 开启后必须暴露 pressed 状态');
