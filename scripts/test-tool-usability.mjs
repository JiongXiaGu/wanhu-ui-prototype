...
assert(topShellCss.includes('.gameplay-top-navigation{') && topShellCss.includes('height:38px') && topShellCss.includes('.gameplay-top-navigation__scene{flex:0 0 84px') && topShellCss.includes('.gameplay-top-navigation__management{flex:1 1 auto') && topShellCss.includes('.gameplay-top-navigation__view{flex:0 0 44px') && topShellCss.includes('flex:0 0 1px;'), 'Top Control Tray Geometry 必须由 gameplay-top-shell.css 以 Flex Group + 真实 Separator 持有');
assert(topShellCss.includes('.gameplay-top-status__world-state{\n  flex:0 0 190px;') && topShellCss.includes('.gameplay-top-status__resources{\n  flex:1 1 auto;') && topShellCss.includes('.gameplay-top-status__time-controls{\n  flex:0 0 190px;'), 'Top Status 必须保持 190 / flexible / 190 的 Flex Row 契约');
...
assert(topShellCss.includes('border:0;') && topShellCss.includes('background:transparent;') && topShellCss.includes('.gameplay-top-navigation__button .ui-icon{width:18px;height:18px;flex:0 0 18px;pointer-events:none}'), 'Top Control Tray Button 必须保留浏览器样式 Reset、18px 图标 Geometry 与固定 Flex Basis');
...
assert(blueprintWorkspaceSource.includes("const PAGE_SIZE = 4") && blueprintWorkspaceSource.includes("id: 'all', label: '全部'") && blueprintWorkspaceSource.includes("id: 'small', label: '小型'") && blueprintWorkspaceSource.includes("id: 'medium', label: '中型'") && blueprintWorkspaceSource.includes("id: 'large', label: '大型'") && blueprintWorkspaceSource.includes('workspace-primary-rail__favorite'), 'Blueprint 左 Rail 必须包含收藏快捷筛选 + 全部 / 小型 / 中型 / 大型，每页 4 张');
...
assert(blueprintWorkspaceSource.includes('新建蓝图') && blueprintWorkspaceSource.includes('onCreate(category)') && blueprintWorkspaceSource.includes('蓝图操作') && blueprintWorkspaceSource.includes("favorite ? '取消收藏' : '收藏'"), 'Blueprint Workspace 必须提供新建入口、全来源常驻操作菜单与收藏切换');
assert(blueprintWorkspaceSource.includes('<span>编辑</span>') && blueprintWorkspaceSource.includes('<span>删除</span>') && !blueprintWorkspaceSource.includes('复制参数'), '我的蓝图在共享收藏动作之外继续提供 编辑 / 删除，不混入材质复制语义');
