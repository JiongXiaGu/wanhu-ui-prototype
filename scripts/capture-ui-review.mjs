...
// Map View Visual Parity：正式 Overlay 使用真实 Spot / Band，不再依赖 CSS Gradient / Filter。
...
for (const mapReview of [
  ...
  if (layerStyle.filter !== 'none') throw new Error('Map View must not depend on browser filter: ' + mapReview.mode + ' ' + layerStyle.filter);
