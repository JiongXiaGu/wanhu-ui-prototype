...
async function assertNoTextOverflow(selector, label) {
  ...
  report.checks.push({ label, overflowCount: failures.length });
}

try {
  // 保留已有三张交付文件名；新增检查只验证这轮明确负责的阅读和控件契约。
  ...
  await open('camera', '.gameplay-context-panel');
  ...
  // 通过真实时间控件进入夜景，不伪造 dataset 或替换背景。
  ...
  await page.keyboard.press('Escape');
  await page.waitForSelector('.gameplay-context-panel', { state: 'detached' });
  await page.getByRole('button', { name: '建筑', exact: true }).click();
  await page.waitForSelector('.workspace--design');
  await settle();
