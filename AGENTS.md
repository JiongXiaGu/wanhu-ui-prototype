...
- World Utility 保持两行：第一行固定地图解锁 / 区域 / 地形 / 配色，第二行放网格 / 范围动作 / Undo / Redo / 批量摧毁。普通 Gameplay 与 Design Workspace 都保持双层；Main Dock 与双层 Utility 统一为 84px 高、bottom=16px，同高同底边。双层 Utility 在该密度下使用 36px 命中区、20px 图标。Workspace / Operation Hints 统一消费 112px Bottom HUD Safe Offset。Tool / Building Selection 仍保持单行 Utility。
