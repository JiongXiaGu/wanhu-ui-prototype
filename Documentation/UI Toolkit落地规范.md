...
- BuildingScheme 当前没有编辑/删除等玩家管理能力，但仍保留共享 `···` 作为收藏和未来资源动作入口，不虚构未实现的编辑操作。

Unity UI Toolkit 迁移建议使用一个 `WorkspaceItemCard.uxml`，由 `Compact / Media` Variant 控制内容槽位和定位；Source Badge、Favorite State 与 Item Menu 继续是共享子结构。Web Prototype 的收藏状态仅用于当前 UI 会话验证，正式 Unity 应由 Catalog/User Metadata 层持久化。
...
- Photography 完成只生成 Preview，再进入 Editor；不会直接创建持久蓝图；
...
- 非 Workspace Hover Card 的默认 `anchor` 仍可按 right → left → top → bottom 评估；Catalog Workspace Rich Hover 则固定显示在当前 Card 上方并水平居中；
- 当前 DesignWorkspace、MaterialPresetWorkspace、BuildingSchemeWorkspace 全部直接消费这一 Workspace Top 规则，Feature 不重复声明定位模式；
