...
详情仍是轻量不可交互浮层，不新增常驻详情栏和锁定功能。所有当前 Catalog Workspace 的 Rich Hover 统一使用框架默认条目锚定：right → left → top → bottom，优先贴近当前条目。DesignWorkspace、MaterialPresetWorkspace、BuildingSchemeWorkspace 不再分别声明定位策略；`workspace-edge` 只保留为未来特殊布局的框架级能力。

定位先将浏览器坐标转换为宿主逻辑坐标，再由纯几何函数计算。默认锚定模式保护当前目标、可见 HUD / 工具区域并经过 Safe Edge / Clamp，允许覆盖相邻目录条目但不能覆盖当前 Anchor；`workspace-edge` 若未来启用，则额外把整个 Workspace 加入保护区域。空间实在不足时标记 constrained，不能宣称任意尺寸都绝无重叠。
