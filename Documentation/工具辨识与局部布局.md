...
详情仍是轻量不可交互浮层，不新增常驻详情栏和锁定功能。所有当前 Catalog Workspace 的 Rich Hover 统一固定显示在当前 Card 上方，并按 Card 水平中心对齐；DesignWorkspace、MaterialPresetWorkspace、BuildingSchemeWorkspace 不再分别声明定位策略。这样同一目录中切换条目时，详情只横向移动，不再出现忽上忽右。`workspace-edge` 只保留为未来特殊布局的框架级能力。

定位先将浏览器坐标转换为宿主逻辑坐标，再由纯几何函数计算。Catalog Workspace 只生成 Card 顶部候选，经过 Safe Edge / Clamp 后显示；允许覆盖相邻目录条目但不能覆盖当前 Anchor。非 Workspace Hover 仍可使用通用多方向 Anchor 规则；`workspace-edge` 若未来启用，则额外把整个 Workspace 加入保护区域。空间实在不足时标记 constrained。
