...
详情仍是轻量不可交互浮层，不新增常驻详情栏和锁定功能。通用 Rich Hover 默认使用条目锚定模式：right → left → top → bottom，优先贴近当前条目；只有需要完整保护目录内容的特殊 Workspace 才显式使用 `workspace-edge`。当前 BuildingSchemeWorkspace 使用 `anchor`，DesignWorkspace 与 MaterialPresetWorkspace 保留 `workspace-edge`。

定位先将浏览器坐标转换为宿主逻辑坐标，再由纯几何函数计算。两种模式都保护当前目标、可见 HUD / 工具区域并经过 Safe Edge / Clamp；`workspace-edge` 额外把整个 Workspace 加入保护区域。空间实在不足时标记 constrained，不能宣称任意尺寸都绝无重叠。
